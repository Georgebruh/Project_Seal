...
const showAlert = (title: string, message: string) => {
  genericAlert.value = { isOpen: true, title, message }
}
const closeAlert = () => {
  genericAlert.value.isOpen = false
}
...
const handlePayMongoCheckout = async () => {
  try {
    ...
    showAlert('Payment Failed', 'Failed to connect to payment gateway. Please try again.')
...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 font-sans relative">
    ...
    <div v-if="genericAlert.isOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm transition-opacity">
      <div class="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-6 text-center transform transition-all">
        ...
        <h3 class="text-xl font-bold text-gray-900 mb-2">{{ genericAlert.title }}</h3>
        <p class="text-gray-500 text-sm mb-6">{{ genericAlert.message }}</p>
        <button 
          @click="closeAlert" 
          class="w-full py-3 bg-gray-900 hover:bg-gray-800 text-white rounded-xl font-bold shadow-sm transition-colors"
        >
          Okay
        </button>
      </div>
